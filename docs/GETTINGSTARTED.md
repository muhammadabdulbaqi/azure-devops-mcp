...
##### Using Token Authentication via a File Path

For scenarios where your token is dynamically provisioned by an external process, sidecar, or host wrapper, use the `header` authentication type with a file containing the bearer token.

1. **Write the bearer token to a file on the local filesystem:**

   ```bash
   echo "your-azure-devops-token" > /tmp/ado-token
   chmod 600 /tmp/ado-token
   ```

2. **Start the MCP server using `header` auth:**

   ```bash
   npx -y @azure-devops/mcp myorg --authentication header --token-path /tmp/ado-token
   ```
   ...
   npx -y @azure-devops/mcp myorg --authentication header
   ```

This approach is useful when the token is generated by another service or when you need the token to be rotated without rebuilding the container.
